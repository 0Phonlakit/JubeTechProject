...
interface IFExamContext {
    ...
    fetchManyExams: (message:string, page?:number, pageSize?:number, sortField?:string, sortOrder?:string) => Promise<void>,
...
export const ExamProvider = ({ children }:{ children: React.ReactNode }) => {
    ...
    // function
    const fetchManyExams = async(message:string = "", page:number = 1, pageSize:number = 10, sortField:string = "title", sortOrder:string = "ascend") => {
        try {
